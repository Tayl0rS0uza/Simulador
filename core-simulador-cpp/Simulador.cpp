...
void Simulador::iniciarSimulacao()
{
    ...
    while(calcularVidaEquipe(1) > 0 && calcularVidaEquipe(2) > 0)
    {
        ...
        if (equipeQueAtaca == 1)
        {
            personagemAtacante = equipe1[std::rand() % equipe1.size()];
            personagemDefesa = equipe2[std::rand() % equipe2.size()];
        }
         if (equipeQueAtaca == 2)
        {
            personagemAtacante = equipe2[std::rand() % equipe2.size()];
            personagemDefesa = equipe1[std::rand() % equipe1.size()];
        }
